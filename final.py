...
# Global variable to track motor state and server logs
motor_running = False
vehicle_stopped = False  # Global variable to track if the vehicle is stopped
...
def update_lcd(line1, line2=""):
    try:
        display.lcd_clear()  # Clear the display before updating
        display.lcd_display_string(line1, 1)  # Update the first line
        display.lcd_display_string(line2, 2)  # Update the second line
        print(f"LCD Updated: {line1} | {line2}")
        sleep(1)  # Brief pause to ensure message visibility
...
def step_motor_continuous(delay=100):
    ...
    if (current_time - last_log_time).total_seconds() > 1:  # Avoid duplicate logs
    ...
    while motor_running:
        for step in step_sequence:
            if not motor_running:
                break
            set_step(*step)
            sleep(delay / 1000)  # Convert delay to seconds
    stop_motor()
...
def stop_motor():
    ...
    motor_running = False
    vehicle_stopped = True  # Mark vehicle as stopped
    set_step(0, 0, 0, 0)  # Stop the motor
    ...
    if vehicle_stopped:
        ...
        while True:
            ...
            if current_finger_state != previous_finger_state:
                if current_finger_state:  # Finger detected
                    ...
                    vehicle_stopped = False  # Mark vehicle as running
                    sleep(2)
                    break
...
def send_sms():
    website_url = 'http://192.168.10.86:5000'  # Base website URL for control
...
def stream_logs():
    ...
    while True:
        sleep(0.1)  # Check for updates periodically
        if len(server_logs) > len(initial_logs):  # Check if new logs are added
            yield f"data: {json.dumps({'logs': server_logs})}\n\n"
...
def stop_motor_web():
    ...
    stop_motor()  # Call the stop motor function
...
def start_motor_web():
    ...
    threading.Thread(target=start_motor_with_ir).start()  # Call the function that waits for IR sensor
    sleep(2)  # Give time for status to update
    return jsonify({"logs": server_logs})
...
def index():
    ...
    """, start_url=start_url, stop_url=stop_url, location_url=location_url, server_logs=server_logs)

initial_logs = server_logs.copy()  # Store initial logs to compare
def run_flask():
